...
/**
 * Returns ALL media files (images, PDFs, PPTx) for a given hackathon slug.
 * GET /api/public/hackathon-media?slug=my-hackathon
 */
export default async function handler(req, res) {
  ...
  try {
    ...
    if (hasBlobToken) {
      try {
        const result = await list({ prefix: `hackathons/${slug}/` });
    ...
    for (const item of images) {
      ...
      if (pathname.startsWith(`hackathons/${slug}/`)) {
        if (item?.url) {
          media.push({
            url: String(item.url),
            pathname,
